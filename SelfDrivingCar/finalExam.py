...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    ...
    delta = [[0, 1], [-1, 1], [1, 1]]
    delta_name = ['-', '/', ',']
    value = [[9999 for row in range(len(road[0]))] for col in range(len(road))]
    policy = [[' ' for row in range(len(road[0]))] for col in range(len(road))]
    change = True
    while change:
        change = False
        for x in range(len(road)):
                ...
                if goal[0] == x and goal[1] == y:
                    ...
                    for a in range(len(delta)):
                        ...
                        if x2 >= 0 and x2 < len(road) and y2 >= 0 and y2 < len(road[0]) and road[x2][y2] != 0:
                            cost_step = 1. / road[x2][y2]
                            if (x != x2):
                                cost_step += lane_change_cost
