...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        t = self.orientation #teta orientation
        x = self.x
        y = self.y
        L = self.length #Longueur entre le pneu avant et arriere
        self.set_noise(bearing_noise, steering_noise, distance_noise)

        a = motion[0] + random.gauss(0, self.steering_noise) #alpha steering angle
        d = motion[1] + random.gauss(0, self.distance_noise) #distance
        B = d/L * tan(a) #beta angle de virage
        ...
        if (B < 0.001):  #(ligne droite)
            ...
            cx = x - sin(t) * R # point x autour duquel la voiture tourne
            cy = y + cos(t) * R # pt y du centre de rotation
            x = cx + sin(t+B) * R
            y = cy - cos(t+B) * R
            t = (t + B) % (2*pi)
        result = robot()
    ...
    def sense(self, noise_enabled=1): #do not change the name of this function
        ...
        for i in range(len(landmarks)):
            if (noise_enabled == 0):
                ...
                Z.append((atan2(landmarks[i][0] - self.y, landmarks[i][1] - self.x) - (self.orientation + random.gauss(0, self.bearing_noise)))%(2*pi))
