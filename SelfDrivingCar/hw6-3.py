...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    print "nombre de ligne /col ", dim
    print "Nombre de positions ", N
    print "Nombre de reperes ", num_landmarks
    #expand Omega
    exp = [i for i in range(0, N*2)]+ [ j for j in range(N*2+2, dim+2)]
    Omega_exp = Omega.expand(dim+2, dim+2, exp, exp)
    #Omega_exp.show("expanded omega")

    #expand Xi
    Xi_exp = Xi.expand(dim+2, 1, exp, [0])
    #Xi_exp.show("expanded Xi")
